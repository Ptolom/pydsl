...
class TestValidate(unittest.TestCase):
    def testBasic(self):
        ...
        tokenized = [x[0] for x in lex(date.alphabet, ascii_encoding, "11/11/2011")]
        self.assertTrue(parser.get_trees(tokenized, True)[0].valid)
        tokenized = [x[0] for x in lex(date.alphabet, ascii_encoding, "11/11//")]
        self.assertFalse(parser.get_trees(tokenized, True)[0].valid)
        self.assertRaises(Exception, lex, date.alphabet, ascii_encoding, "11/11/ab")
